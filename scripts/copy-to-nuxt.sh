...
translate_and_copy_config() {
  ...
  # Create directory for prime language and copy the original configuration
  mkdir -p "$NUXTDIR/$NORMALIZED_SPEC_NAME/$PRIMELANGUAGE"
  node /app/convert-config.js -i "$CONFIG_NAME-multilang.json" -l "$PRIMELANGUAGE" -o "$NUXTDIR/$NORMALIZED_SPEC_NAME/$PRIMELANGUAGE/configuration.json" >>"$ERROR_FILE"
